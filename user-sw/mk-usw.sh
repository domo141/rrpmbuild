...
# Last modified: Fri 11 Oct 2024 18:05:16 +0300 too
...
v=$(perl -e 'use strict; use warnings;
  open P, "-|", qw/git lsog -99 --format=%ci/ or die $!;
  $_ = <P>; /^(\d\d\d\d)-/ or die; my ($y, $c) = ($1, 1);
  while (<P>) {
    /^(\d\d\d\d)-/ or die;
    last unless $1 eq $y;
    $c++
  }
  printf "%02d.%02d", $y - 2000, $c') || v=24.21 # close?
  #v=`date --date='now - 13 years' +%y.%m`
